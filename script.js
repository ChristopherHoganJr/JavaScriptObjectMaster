...
const pokeNamesOver100 = pokémon.filter((p) => p.id > 100).map((p) => p.name);
const pokeNames1TypePoison = pokémon
  .filter((p) => p.types.includes("poison"))
  .filter((p) => p.types.length === 1);
const pokemonSecondTypeFlying = pokémon
  .filter((p) => p.types[1] === "flying")
  .map((p) => p.types[0]);
const normalPokemonTypes = pokémon.filter((p) =>
